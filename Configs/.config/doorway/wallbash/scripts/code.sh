...
# shellcheck source=$HOME/.local/bin/doorway-shell
...
cacheDir="${cacheDir:-$XDG_CACHE_HOME/doorway}"
...
for i in "${!codeConf[@]}"; do
    ...
    extTheme="$(
        "$LIB_DIR/doorway/parse.json.py" -CRQ '.["workbench.colorTheme"]' "${codeConf[i]}/User/settings.json"
    )"
    if [ "${extTheme}" != "${set_theme}" ]; then
        "$LIB_DIR/doorway/parse.json.py" -CU '.["workbench.colorTheme"]' "${set_theme}" "${codeConf[i]}/User/settings.json"
