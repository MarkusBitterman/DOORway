...
_doorwayde_shell() {
    ...
    local built_in_commands doorwayde_scripts wallbash_scripts
    ...
    if (( $+commands[doorwayde-shell] )); then
        local scripts_raw
        scripts_raw=(${(f)"$(doorwayde-shell --list-script 2>/dev/null)"})
        doorwayde_scripts=(${scripts_raw[@]%.*})  # Remove extensions
    ...
    if [[ $CURRENT -eq 2 ]]; then
        ...
        local all_commands=($built_in_commands $doorwayde_scripts)
...
compdef _doorwayde_shell doorwayde-shell