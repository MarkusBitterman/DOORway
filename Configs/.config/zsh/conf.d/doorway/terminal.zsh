...
function _dedup_zsh_plugins {
    ...
    # Load Plugins
    doorway_plugins=(git zsh-256color zsh-autosuggestions zsh-syntax-highlighting)
    plugins+=("${plugins[@]}" "${doorway_plugins[@]}")
    # Deduplicate plugins
    plugins=("${plugins[@]}")
    plugins=($(printf "%s\n" "${plugins[@]}" | sort -u))
...
function _load_deferred_plugin_system_by_doorway() {

    # Exit early if DOORWAY_ZSH_DEFER is not set to 1
    if [[ "${DOORWAY_ZSH_DEFER}" != "1" ]]; then
        unset -f _load_deferred_plugin_system_by_doorway
        return
    fi
...
function _load_compinit() {
    ...
    if [[ -n ${ZDOTDIR}/.zcompdump(#qN.mh+${DOORWAY_ZSH_COMPINIT_CHECK:-1}) ]]; then
        compinit
    else
        compinit -C
    fi
...
function _load_prompt() {
    ...
    if ! source ${ZDOTDIR}/prompt.zsh >/dev/null 2>&1; then
        [[ -f $ZDOTDIR/conf.d/doorway/prompt.zsh ]] && source $ZDOTDIR/conf.d/doorway/prompt.zsh
    fi
}
...
DOORWAY_ZSH_DEFER="1"      #Unset this variable in $ZDOTDIR/user.zsh to disable DOORway's deferred Zsh loading.
DOORWAY_ZSH_PROMPT="1"     #Unset this variable in $ZDOTDIR/user.zsh to disable DOORway's prompt customization.
DOORWAY_ZSH_NO_PLUGINS="0" #Set this variable to "1" in $ZDOTDIR/user.zsh to disable DOORway's Zsh plugin loading.
...
# DOORway Package Manager
PM_COMMAND=(doorway-shell pm)

# Optionally load user configuration // useful for customizing the shell without modifying the main file
if [[ -f $HOME/.doorway.zshrc ]]; then
    source $HOME/.doorway.zshrc # for backward compatibility
...
if [[ ${DOORWAY_ZSH_NO_PLUGINS} != "1" ]]; then
    _dedup_zsh_plugins
    if [[ "$DOORWAY_ZSH_OMZ_DEFER" == "1" ]] && [[ -r $ZSH/oh-my-zsh.sh ]]; then
        # Loads the buggy deferred oh-my-zsh plugin system by DOORway // This is only for oh-my-zsh and compatibility
        _load_deferred_plugin_system_by_doorway
