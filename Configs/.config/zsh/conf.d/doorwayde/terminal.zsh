...
function _dedup_zsh_plugins {
    ...
    # Load Plugins
    doorwayde_plugins=(git zsh-256color zsh-autosuggestions zsh-syntax-highlighting)
    plugins+=("${plugins[@]}" "${doorwayde_plugins[@]}")
    # Deduplicate plugins
    plugins=("${plugins[@]}")
    plugins=($(printf "%s\n" "${plugins[@]}" | sort -u))
...
function _load_deferred_plugin_system_by_doorwayde() {

    # Exit early if DOORWAYDE_ZSH_DEFER is not set to 1
    if [[ "${DOORWAYDE_ZSH_DEFER}" != "1" ]]; then
        unset -f _load_deferred_plugin_system_by_doorwayde
        return
    fi
...
function _load_compinit() {
    ...
    if [[ -n ${ZDOTDIR}/.zcompdump(#qN.mh+${DOORWAYDE_ZSH_COMPINIT_CHECK:-1}) ]]; then
        compinit
    else
        compinit -C
    fi
...
function _load_prompt() {
    ...
    if ! source ${ZDOTDIR}/prompt.zsh >/dev/null 2>&1; then
        [[ -f $ZDOTDIR/conf.d/doorwayde/prompt.zsh ]] && source $ZDOTDIR/conf.d/doorwayde/prompt.zsh
    fi
}
...
DOORWAYDE_ZSH_DEFER="1"      #Unset this variable in $ZDOTDIR/user.zsh to disable DOORwayDE's deferred Zsh loading.
DOORWAYDE_ZSH_PROMPT="1"     #Unset this variable in $ZDOTDIR/user.zsh to disable DOORwayDE's prompt customization.
DOORWAYDE_ZSH_NO_PLUGINS="0" #Set this variable to "1" in $ZDOTDIR/user.zsh to disable DOORwayDE's Zsh plugin loading.
...
# DOORwayDE Package Manager
PM_COMMAND=(doorwayde-shell pm)

# Optionally load user configuration // useful for customizing the shell without modifying the main file
if [[ -f $HOME/.doorwayde.zshrc ]]; then
    source $HOME/.doorwayde.zshrc # for backward compatibility
...
if [[ ${DOORWAYDE_ZSH_NO_PLUGINS} != "1" ]]; then
    _dedup_zsh_plugins
    if [[ "$DOORWAYDE_ZSH_OMZ_DEFER" == "1" ]] && [[ -r $ZSH/oh-my-zsh.sh ]]; then
        # Loads the buggy deferred oh-my-zsh plugin system by DOORwayDE // This is only for oh-my-zsh and compatibility
        _load_deferred_plugin_system_by_doorwayde
