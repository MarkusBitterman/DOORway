...
class DoorwayConfig:
    """Handle DOORway configuration loading and parsing"""

    def __init__(self):
        self.config = self._load_config()

    def _load_config(self):
        """Load DOORway configuration from $XDG_STATE_HOME/doorway/config"""
        state_dir = os.path.expanduser(os.getenv("XDG_STATE_HOME", "~/.local/state"))
        config_file = os.path.join(state_dir, "doorway", "config")
        ...
        try:
            ...
            print(f"Warning: Could not load DOORway config: {e}", file=sys.stderr)

        return config

    def get_value(self, key, default=None):
        """Get value from DOORway config, falling back to environment, then default"""
...
class CavaServer:
    ...
    def __init__(self):
        ...
        self.socket_file = os.path.join(self.runtime_dir, "doorway", "cava.sock")
        self.pid_file = os.path.join(self.runtime_dir, "doorway", "cava.pid")
        self.temp_dir = Path(os.path.join(self.runtime_dir, "doorway"))
    ...
    def _reload_cava_process(self):
        ...
        doorway_config = DoorwayConfig()
        bars = int(doorway_config.get_value("CAVA_BARS", 16))
        range_val = int(doorway_config.get_value("CAVA_RANGE", 15))
        channels = doorway_config.get_value("CAVA_CHANNELS", "stereo")
        reverse = doorway_config.get_value("CAVA_REVERSE", 0)
        try:
            reverse = int(reverse)
        except Exception:
    ...
    def _create_cava_config(self, bars=16, range_val=15, channels="stereo", reverse=0, prefix=""):
        """Create cava configuration file with channels and reverse support, using DoorwayConfig with or without prefix as appropriate"""
        doorway_config = DoorwayConfig()

        if prefix:
            config_channels = doorway_config.get_value(f"CAVA_{prefix}_CHANNELS")
            config_reverse = doorway_config.get_value(f"CAVA_{prefix}_REVERSE")
        else:
            config_channels = doorway_config.get_value("CAVA_CHANNELS")
            config_reverse = doorway_config.get_value("CAVA_REVERSE")
...
class CavaClient:
    ...
    @staticmethod
    def parse_command_config(doorway_config, command, args):
        ...
        if hasattr(args, "bar_array") and args.bar_array:
            ...
            bar_array = doorway_config.get_value(f"{prefix}_BAR_ARRAY")
            if bar_array and isinstance(bar_array, list):
                bar_chars = bar_array
            else:
                bar_chars = args.bar or doorway_config.get_value(f"{prefix}_BAR", "▁▂▃▄▅▆▇█")
        ...
        width = (
            ...
            else int(doorway_config.get_value(f"{prefix}_WIDTH", "0") or 0)
        )
        if not width:
        ...
        if args.stb is not None:
            ...
            standby_mode = doorway_config.get_value(f"{prefix}_STANDBY", "0")
...
def main():
    ...
    if args.command == "manager":
        ...
        bar_chars, width, standby_mode = CavaClient.parse_command_config(
            doorway_config, args.command, args
        )
