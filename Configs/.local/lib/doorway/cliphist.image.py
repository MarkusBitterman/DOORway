...
def main():
    ...
    for line in lines:
        ...
        elif os.getenv("DOORWAY_CLIPHIST_IMAGE_ONLY", "").lower() not in (
            "true",
            "1",
            "yes",
        ):
            # image path is ~/.cache/doorway/wallbash/pry1.svg but use xdg
            img_path = os.getenv("XDG_CACHE_HOME", os.path.expanduser("~/.cache"))
            img_path = os.path.join(img_path, "doorway", "wallbash", "pry1.svg")
            print(f"{line}\0icon\x1f{img_path}")
