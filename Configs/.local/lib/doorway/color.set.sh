...
fn_wallbash() {
    ...
    [ -f "$DOORWAY_STATE_HOME/state" ] && source "$DOORWAY_STATE_HOME/state"
    ...
    export WALLBASH_SCRIPTS confDir doorwayConfDir cacheDir thmbDir dcolDir iconsDir themesDir fontsDir wallbashDirs enableWallDcol DOORWAY_THEME_DIR DOORWAY_THEME GTK_ICON GTK_THEME CURSOR_THEME
...
set -a
source "$dcol_file"
if [ -f "$DOORWAY_THEME_DIR/theme.dcol" ] && [ "$enableWallDcol" -eq 0 ]; then
    source "$DOORWAY_THEME_DIR/theme.dcol"
    print_log -sec "wallbash" -stat "override" "dominant colors from $DOORWAY_THEME theme"
    print_log -sec "wallbash" -stat " NOTE" "Remove \"$DOORWAY_THEME_DIR/theme.dcol\" to use wallpaper dominant colors"
fi
...
if [ "$enableWallDcol" -eq 0 ] && [[ $reload_flag -eq 1 ]]; then
    print_log -sec "wallbash" -stat "apply $dcol_mode colors" "$DOORWAY_THEME theme"
    mapfile -d '' -t deployList < <(find -H "$DOORWAY_THEME_DIR" -type f -name "*.theme" -print0)
    while read -r pKey; do
        fKey="$(find -H "$DOORWAY_THEME_DIR" -type f -name "$(basename "${pKey%.dcol}.theme")")"
