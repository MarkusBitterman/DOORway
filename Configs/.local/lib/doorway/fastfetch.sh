...
cacheDir="${XDG_CACHE_HOME:-$HOME/.cache}/doorway"
image_dirs=()
doorway_distro_logo=$iconDir/Wallbash-Icon/distro/$LOGO
case $1 in
    logo)
        random() {
            (   
                ...
                if [ -n "$DOORWAY_THEME" ] && [ -d "$confDir/doorway/themes/$DOORWAY_THEME/logo" ]; then
                    image_dirs+=("$confDir/doorway/themes/$DOORWAY_THEME/logo")
                fi
                [ -f "$doorway_distro_logo" ] && echo "$doorway_distro_logo"
...
options:
  ...
  --theme   Display a logo inside the doorway theme directory
            ...
            for arg in "$@"; do
                case $arg in
                    ...
                    --prof)
                        ...
                        ;;
                    --os)
                        [ -f "$doorway_distro_logo" ] && image_dirs+=("$doorway_distro_logo")
                        ;;
                    --local)
                        image_dirs+=("$confDir/fastfetch/logo")
                        ;;
                    --wall)
                        ...
                        ;;
                    --theme) if
                        [ -n "$DOORWAY_THEME" ] && [ -d "$confDir/doorway/themes/$DOORWAY_THEME/logo" ]
                    then
                        image_dirs+=("$confDir/doorway/themes/$DOORWAY_THEME/logo")
                    fi ;;
                esac
