...
case ${style:-5} in
    ...
    "monitor_info=(\(.width) \(.height) \(.scale) \(.x) \(.y)) reserved_info=(\(.reserved | join(" ")))"')"
        ...
        BG=$HOME/.local/share/doorway/rofi/assets/steamdeck_holographic.png
        BGfx=$HOME/.cache/doorway/landing/steamdeck_holographic_${monitor_width}x$monitor_height.png
        if [ ! -e "$BGfx" ]; then
...
case "$backend" in
    steam)
        backend_command=("${XDG_STATE_HOME:-$HOME/.local/state}/doorway/python_env/bin/python" "$LIB_DIR/doorway/gamelauncher/steam.py" --rofi-string)
        ;;
    lutris)
        backend_command=("${XDG_STATE_HOME:-$HOME/.local/state}/doorway/python_env/bin/python" "$LIB_DIR/doorway/gamelauncher/lutris.py" --rofi-string)
        ;;
    *)
        backend_command=("${XDG_STATE_HOME:-$HOME/.local/state}/doorway/python_env/bin/python" "$LIB_DIR/doorway/gamelauncher/catalog.py" --rofi-string)
