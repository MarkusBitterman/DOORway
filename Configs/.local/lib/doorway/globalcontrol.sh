...
export DOORWAY_CONFIG_HOME="$XDG_CONFIG_HOME/doorway"
export DOORWAY_DATA_HOME="$XDG_DATA_HOME/doorway"
export DOORWAY_CACHE_HOME="$XDG_CACHE_HOME/doorway"
export DOORWAY_STATE_HOME="$XDG_STATE_HOME/doorway"
export DOORWAY_RUNTIME_DIR="$XDG_RUNTIME_DIR/doorway"
export ICONS_DIR="$XDG_DATA_HOME/icons"
export FONTS_DIR="$XDG_DATA_HOME/fonts"
export THEMES_DIR="$XDG_DATA_HOME/themes"
export scrDir="${LIB_DIR:-$HOME/.local/lib}/doorway"
export confDir="${XDG_CONFIG_HOME:-$HOME/.config}"
export doorwayConfDir="$DOORWAY_CONFIG_HOME"
export cacheDir="$DOORWAY_CACHE_HOME"
export thmbDir="$DOORWAY_CACHE_HOME/thumbs"
export dcolDir="$DOORWAY_CACHE_HOME/dcols"
...
get_hashmap() {
    ...
    if [ -z "${#wallList[@]}" ] || [[ ${#wallList[@]} -eq 0 ]]; then
        if [[ $skipStrays -eq 1 ]]; then
            ...
            [ -n "$no_notify" ] && notify-send -a "DOORway Alert" "WARNING: No compatible wallpapers found in: ${no_wallpapers[*]}"
            exit 1
        fi
    fi
...
get_themes() {
    ...
    while read -r thmDir; do
        ...
        thmListS+=("${thmDir##*/}")
    done < <(find -H "$DOORWAY_CONFIG_HOME/themes" -mindepth 1 -maxdepth 1 -type d)
...
export_doorway_config() {
    local user_conf_state="$XDG_STATE_HOME/doorway/staterc"
    local user_conf="$XDG_STATE_HOME/doorway/config"
...
if [ -z "$DOORWAY_THEME" ] || [ ! -d "$DOORWAY_CONFIG_HOME/themes/$DOORWAY_THEME" ]; then
    get_themes
    DOORWAY_THEME="${thmList[0]}"
fi
DOORWAY_THEME_DIR="$DOORWAY_CONFIG_HOME/themes/$DOORWAY_THEME"
WALLBASH_DIRS=(
    "$XDG_CONFIG_HOME/wallbash"
    "$XDG_CONFIG_HOME/doorway/wallbash"
    "$XDG_DATA_HOME/wallbash"
    "$XDG_DATA_HOME/doorway/wallbash"
    "/usr/local/share/doorway/wallbash"
...
export hypr_border=${hypr_border:-${DOORWAY_BORDER_RADIUS:-2}}
export hypr_width=${hypr_width:-${DOORWAY_BORDER_WIDTH:-2}}
pkg_installed() {
    local pkgIn=$1
    if command -v "$pkgIn" &>/dev/null; then
        ...
        return 0
    elif doorway-shell pm.sh pq "$pkgIn" &>/dev/null; then
        return 0
    else
        return 1
    fi
...
set_conf() {
    local varName="$1"
    local varData="$2"
    touch "$XDG_STATE_HOME/doorway/staterc"
    if [ "$(grep -c "^$varName=" "$XDG_STATE_HOME/doorway/staterc")" -eq 1 ]; then
        sed -i "/^$varName=/c$varName=\"$varData\"" "$XDG_STATE_HOME/doorway/staterc"
    else
        echo "$varName=\"$varData\"" >>"$XDG_STATE_HOME/doorway/staterc"
    fi
}
set_hash() {
    local hashImage="$1"
...
check_package() {
    local lock_file="${XDG_RUNTIME_DIR:-/tmp}/doorway/__package.lock"
    mkdir -p "${XDG_RUNTIME_DIR:-/tmp}/doorway"
    if [ -f "$lock_file" ]; then
        return 0
    fi
    for pkg in "$@"; do
...
get_hyprConf() {
    local hyArg="$1"
    local file="${2:-"$DOORWAY_THEME_DIR/hypr.theme"}"
    local hyVar="$hyArg"
    local hyType=""
    ...
    if [ -z "$gsVal" ] || [[ $gsVal == \$* ]]; then
        ...
        *) grep "^[[:space:]]*\$default.$hyVar\s*=" \
            "XDG_DATA_HOME/doorway/doorway.conf" \
            "$XDG_DATA_HOME/doorway/hyprland.conf" \
            "/usr/local/share/doorway/doorway.conf" \
            "/usr/local/share/doorway/hyprland.conf" \
            "/usr/share/doorway/doorway.conf" \
            "/usr/share/doorway/hyprland.conf" 2>/dev/null | cut -d '=' -f2 | sed 's/^[[:space:]]*//;s/[[:space:]]*$//' | head -n 1 ;;
        esac
    else
        echo "$gsVal"
    fi
...
paste_string() {
    ...
    ignore_paste_file="$DOORWAY_STATE_HOME/ignore.paste"
...
accepted_mime_types() {
    ...
    for mime_type in "${mime_types_array[@]}"; do
        if file --mime-type -b "$file" | grep -q "^$mime_type"; then
            ...
            notify-send -u critical -a "DOORway-Alert" "File type not supported for this wallpaper backend."
        fi
    done
}
dconf_write() {
