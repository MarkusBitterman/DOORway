...
def load_toml_file(toml_file):
    try:
        ...
        notify.send("DOORway Error", error_message)
        return None
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="The input TOML file to parse. Default is $XDG_CONFIG_HOME/doorway/config.toml",
    )
    parser.add_argument(
        "--env",
        default=os.path.join(
        ...
        help="The output environment file. Default is $XDG_STATE_HOME/doorway/config",
    )
    parser.add_argument(
        "--hypr",
        default=os.path.join(
            xdg_state_home(),
            "doorway/hyprland.conf",
        ),
