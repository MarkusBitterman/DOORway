#!/usr/bin/env python3
"""
DOORway's LuaJIT environment manager
...
DOORWAY_STATE_DIR = os.path.join(XDG_STATE_HOME, "doorway")
LUA_ENV_DIR = os.path.join(XDG_STATE_HOME, "doorway", "lua_env")
...
ROCKS_SNAPSHOT = os.path.join(DOORWAY_STATE_DIR, "luarocks_env.json")
...
def ensure_state_dir():
    os.makedirs(DOORWAY_STATE_DIR, exist_ok=True)
...
def main():
    parser = argparse.ArgumentParser(
        prog="lua_env",
        usage="lua_env.py [command] <options>",
        description="DOORway's LuaJIT environment manager",
