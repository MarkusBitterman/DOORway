...
def get_venv_path() -> str:
    ...
    return os.path.join(str(xdg_base_dirs.xdg_state_home()), "doorway", "python_env")
...
def run_uv(args, venv_path: str = None, notify_msg: str = None, stream: bool = False) -> subprocess.CompletedProcess[str]:
    ...
    try:
        ...
        notify.send("DOORway UV", str(e), urgency="critical")
        raise
    ...
    if notify_msg:
        notify.send("DOORway UV", notify_msg, replace_id=9)
    ...
    if stream:
        ...
        if result.returncode != 0:
            notify.send("DOORway UV", "Command failed", urgency="critical")
    ...
    if result.returncode != 0:
        ...
        notify.send("DOORway UV", f"Error:\n{err}", urgency="critical")
...
def create_venv() -> None:
    ...
    if os.path.exists(venv_path):
        if is_venv_valid(venv_path):
            notify.send("DOORway UV", "ℹ️ Virtual environment already exists and is valid")
            return
        notify.send("DOORway UV", "⚠️ Broken venv detected, recreating…")
        destroy_venv(venv_path)
    run_uv(["sync"], notify_msg="⏳ Creating virtual environment...")
    notify.send("DOORway UV", "✅ Virtual environment ready")
...
def destroy_venv(venv_path=None) -> None:
    ...
    if os.path.exists(venv_path):
        ...
        notify.send("DOORway UV", "🗑️ Virtual environment removed")
...
def rebuild_venv() -> None:
    ...
    run_uv(["sync"])
    notify.send("DOORway UV", "✅ Rebuild complete")
...
def sync_packages() -> None:
    ...
    notify.send("DOORway UV", "✅ Dependencies are up to date", replace_id=9)
...
def install_package(package: str | Iterable[str]) -> None:
    ...
    if not pkgs:
        notify.send("DOORway UV", "No packages specified for installation", urgency="warning")
        return

    notify.send("DOORway UV", f"Installing {', '.join(pkgs)}...")
    try:
        run_uv(["add"] + pkgs, stream=True)
    except RuntimeError as e:
        notify.send("DOORway UV", f"Error installing packages: {e}", urgency="critical")
        raise
...
def uninstall_package(package: str | Iterable[str]) -> None:
    ...
    if not pkgs:
        notify.send("DOORway UV", "No packages specified for uninstallation", urgency="warning")
        return

    notify.send("DOORway UV", f"Uninstalling {', '.join(pkgs)}...")
    try:
        run_uv(["remove"] + pkgs, stream=True)
    except RuntimeError as e:
        notify.send("DOORway UV", f"Error uninstalling packages: {e}", urgency="critical")
        raise
...
def v_import(module_name: str, auto_install: bool = True, extra: str = None) -> ModuleType:
    ...
    try:
        ...
        notify.send("DOORway UV", f"Installing missing module: {module_name}")
        if extra:
        ...
        try:
            ...
            notify.send("DOORway UV", f"{module_name} installed successfully")
            return module
...
def main(argv) -> None:
    parser = argparse.ArgumentParser(
        ...
        description="DOORway's Python virtual environment manager",
    ...
    uv_p = subparsers.add_parser("uv", help="Run a raw uv command. Use --doorway to scope DOORway's venv")
    uv_p.add_argument("--doorway", action="store_true", help="Run within DOORway's virtualenv context")
