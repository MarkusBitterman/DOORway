...
rofiStyleDir="$SHARE_DIR/doorway/rofi/themes"
rofiAssetDir="$SHARE_DIR/doorway/rofi/assets"
...
if [ -n "$RofiSel" ]; then
    ...
    notify-send -a "DOORway Alert" -r 2 -t 2200 -i "$rofiAssetDir/$RofiSel.png" " style $RofiSel applied..."
fi
if [ -n "$ROFI_LAUNCH_STYLE" ]; then
    notify-send -a "DOORway Alert" -r 3 -u critical "Style: '$ROFI_LAUNCH_STYLE' is explicitly set, remove it in ~/.config/doorway/config.toml for changes to take effect."
fi
