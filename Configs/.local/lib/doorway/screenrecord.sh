...
if ! command -v "$RECORDER" &> /dev/null; then
    notify-send -a "DOORway Alert" "No screen recorder found. Try installing wl-screenrec or wf-recorder."
...
Usage: 'doorway-shell screenrecord' [option]
...
Example:
        'doorway-shell screenrecord' --start -- --audio --codec libx264
...
handle_recording() {
    ...
    if [[ -z $GEOM ]]; then
        "$LIB_DIR/doorway/screenshot/grimblast" save active "$tmp_thumbnail"
    else
        grim -g "$GEOM" "$tmp_thumbnail"
    fi
    "$RECORDER" "${parameters[@]}" -f "$save_file_path"
    notify-send -a "DOORway Alert" "$RECORDER: Recording saved at $save_file_path" -i "$tmp_thumbnail"
}
while [[ $# -gt 0 ]]; do
    case "$1" in
        --file)
        ...
        --quit)
            killall "$RECORDER"
            notify-send -a "DOORway Alert" "Recording stopped"
            exit 0
            ;;
        --help)
            USAGE
