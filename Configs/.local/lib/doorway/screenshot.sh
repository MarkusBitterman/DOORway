...
[[ -f "${LIB_DIR}/doorway/shutils/l10n.sh" ]] && source "${LIB_DIR}/doorway/shutils/l10n.sh"
...
temp_screenshot=${XDG_RUNTIME_DIR:-/tmp}/doorway_screenshot.png
...
take_screenshot() {
    ...
    shift
    local extra_args=("$@")
    if "$LIB_DIR/doorway/screenshot/grimblast" "${extra_args[@]}" copysave "$mode" "$temp_screenshot"; then
        [[ ${SCREENSHOT_ANNOTATION_ENABLED} == false ]] && return 0
        if ! "$annotation_tool" "${annotation_args[@]}"; then
            send_notifs -r 9 -a "DOORway Alert" "Screenshot Error" "Failed to open annotation tool"
            return 1
        fi
    else
        send_notifs -a "DOORway Alert" "Screenshot Error" "Failed to take screenshot"
        return 1
    fi
}
ocr_screenshot() {
    ...
    if "$LIB_DIR/doorway/screenshot/grimblast" "${extra_args[@]}" copysave "$mode" "$temp_screenshot"; then
        ...
        if ! ocr_extract "$temp_screenshot"; then
            send_notifs -r 9 -a "DOORway Alert" "OCR: extraction error" -e -i "dialog-error"
            return 1
        fi
    else
        send_notifs -a "DOORway Alert" "OCR: screenshot error" -e -i "dialog-error"
        return 1
    fi
}
qr_screenshot() {
    ...
    local extra_args=("$@")
    if "$LIB_DIR/doorway/screenshot/grimblast" "${extra_args[@]}" copysave "$mode" "$temp_screenshot"; then
        source "${LIB_DIR}/doorway/shutils/qr.sh"
        ...
        if ! qr_extract "$temp_screenshot"; then
            send_notifs -r 9 -a "DOORway Alert" "QR: extraction error" -e -i "dialog-error"
            return 1
        fi
    else
        send_notifs -a "DOORway Alert" "QR: screenshot error" -e -i "dialog-error"
        return 1
    fi
}
...
if [ -f "$save_dir/$save_file" ]; then
    send_notifs -r 9 -a "DOORway Alert" -i "$save_dir/$save_file" "saved in $save_dir"
fi
