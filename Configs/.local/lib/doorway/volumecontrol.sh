...
notify_vol() {
    ...
    [[ $isNotify == true ]] && notify-send -a "DOORway Notify" -r 8 -t 2000 -i "$ico" "$vol$bar" "$nsink"
}
notify_mute() {
    ...
    if [ "$mute" == "true" ]; then
        [[ $isNotify == true ]] && notify-send -a "DOORway Notify" -r 8 -t 2000 -i "$icodir/muted-$dvce.svg" "muted" "$nsink"
    else
        [[ $isNotify == true ]] && notify-send -a "DOORway Notify" -r 8 -t 2000 -i "$icodir/unmuted-$dvce.svg" "unmuted" "$nsink"
    fi
}
change_volume() {
    local action=$1
