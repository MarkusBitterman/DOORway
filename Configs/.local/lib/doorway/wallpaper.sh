...
source "$LIB_DIR/doorway/wallpaper/help.sh"
source "$LIB_DIR/doorway/wallpaper/core.sh"
source "$LIB_DIR/doorway/wallpaper/select.sh"
...
run_cache_command() {
    ...
    case "$cache_mode" in
    current)
        "$LIB_DIR/doorway/wallpaper/cache.sh" commence
        ;;
    w | wall)
        ...
        "$LIB_DIR/doorway/wallpaper/cache.sh" commence -w "$cache_arg"
        ;;
    t | theme)
        if [ -z "$cache_arg" ]; then
        ...
        "$LIB_DIR/doorway/wallpaper/cache.sh" commence -t "$cache_arg"
        ;;
    f | full | all)
        "$LIB_DIR/doorway/wallpaper/cache.sh" commence -f
        ;;
    *)
...
setup_wallpaper_targets() {
    ...
    if [ "$set_as_global" == "true" ]; then
        wallSet="$DOORWAY_THEME_DIR/wall.set"
        wallCur="$DOORWAY_CACHE_HOME/wall.set"
        wallSqr="$DOORWAY_CACHE_HOME/wall.sqre"
        wallTmb="$DOORWAY_CACHE_HOME/wall.thmb"
        wallBlr="$DOORWAY_CACHE_HOME/wall.blur"
        wallQad="$DOORWAY_CACHE_HOME/wall.quad"
        wallDcl="$DOORWAY_CACHE_HOME/wall.dcol"
    elif [ -n "$wallpaper_backend" ]; then
        mkdir -p "$DOORWAY_CACHE_HOME/wallpapers"
        wallCur="$DOORWAY_CACHE_HOME/wallpapers/$wallpaper_backend.png"
        wallSet="$DOORWAY_THEME_DIR/wall.$wallpaper_backend.png"
    else
        wallSet="$DOORWAY_THEME_DIR/wall.set"
    fi
    if [ ! -e "$wallSet" ]; then
...
main() {
    ...
    if [ -n "$wallpaper_setter_flag" ]; then
        ...
        select)
            ...
            if [ "$output_flag" != "true" ]; then
                "$LIB_DIR/doorway/wallpaper/cache.sh" commence &>/dev/null &
            fi
            Wall_Select
    ...
    if [ -f "$LIB_DIR/doorway/wallpaper.$wallpaper_backend.sh" ] && [ -n "$wallpaper_backend" ]; then
        print_log -sec "wallpaper" "Using backend: $wallpaper_backend"
        "$LIB_DIR/doorway/wallpaper.$wallpaper_backend.sh" "$wallSet"
    else
        if command -v "wallpaper.$wallpaper_backend.sh" >/dev/null; then
            ...
            print_log -warn "wallpaper" "Created: $DOORWAY_CACHE_HOME/wallpapers/$wallpaper_backend.png instead"
        fi
    fi
    if [ "$wallpaper_setter_flag" == "select" ]; then
        if [ -e "$(readlink -f "$wallSet")" ]; then
            if [ "$set_as_global" == "true" ]; then
                notify-send -a "DOORway Alert" -i "$selected_thumbnail" "$selected_wallpaper"
            else
                notify-send -a "DOORway Alert" -i "$selected_thumbnail" "$selected_wallpaper set for $wallpaper_backend"
            fi
        else
            notify-send -a "DOORway Alert" "Wallpaper not found"
        fi
    fi
}
if [ -z "$*" ]; then
