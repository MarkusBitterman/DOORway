...
notify-send "DEPRECATION NOTICE: swww backend is deprecated, please switch to awww or other supported backends. See 'doorway-shell wallpaper --help' for more info."

selected_wall="${1:-"$$DOORWAY_CACHE_HOME/wall.set"}"
lockFile="$XDG_RUNTIME_DIR/doorway/$(basename "$0").lock"
if [ -e "$lockFile" ]; then
    cat << EOF
