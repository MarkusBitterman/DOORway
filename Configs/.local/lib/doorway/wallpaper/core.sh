...
Wall_Json() {
    setIndex=0
    [ ! -d "$DOORWAY_THEME_DIR" ] && echo "ERROR: \"$DOORWAY_THEME_DIR\" does not exist" && exit 0
    if [ -d "$DOORWAY_THEME_DIR/wallpapers" ]; then
        wallPathArray=("$DOORWAY_THEME_DIR/wallpapers")
    else
        wallPathArray=("$DOORWAY_THEME_DIR")
    fi
    wallPathArray+=("${WALLPAPER_CUSTOM_PATHS[@]}")
    [ -d "${XDG_PICTURES_DIR%/}/Wallpapers/${DOORWAY_THEME}" ] && wallPathArray+=("${XDG_PICTURES_DIR%/}/Wallpapers/${DOORWAY_THEME}")
    get_hashmap "${wallPathArray[@]}"
    wallListJson=$(printf '%s\n' "${wallList[@]}" | jq -R . | jq -s .)
    wallHashJson=$(printf '%s\n' "${wallHash[@]}" | jq -R . | jq -s .)
    jq -n --argjson wallList "$wallListJson" --argjson wallHash "$wallHashJson" --arg cacheHome "${DOORWAY_CACHE_HOME:-$HOME/.cache/doorway}" '
...
Wall_Cache() {
    ...
    if [ "$set_as_global" == "true" ]; then
        ...
        "$LIB_DIR/doorway/wallpaper/cache.sh" commence -w "${wallList[setIndex]}" &> /dev/null
        "$LIB_DIR/doorway/color.set.sh" "${wallList[setIndex]}" &
