...
class DoorwaydeConfig:
    """Handle DOORwayDE configuration loading and parsing"""

    def __init__(self):
        self.config = self._load_config()

    def _load_config(self):
        """Load DOORwayDE configuration from $XDG_STATE_HOME/doorwayde/config"""
        state_dir = os.path.expanduser(os.getenv("XDG_STATE_HOME", "~/.local/state"))
        config_file = os.path.join(state_dir, "doorwayde", "config")
        ...
        try:
            ...
            print(f"Warning: Could not load DOORwayDE config: {e}", file=sys.stderr)

        return config

    def get_value(self, key, default=None):
        """Get value from DOORwayDE config, falling back to environment, then default"""
...
class CavaServer:
    ...
    def __init__(self):
        ...
        self.socket_file = os.path.join(self.runtime_dir, "doorwayde", "cava.sock")
        self.pid_file = os.path.join(self.runtime_dir, "doorwayde", "cava.pid")
        self.temp_dir = Path(os.path.join(self.runtime_dir, "doorwayde"))
    ...
    def _reload_cava_process(self):
        ...
        bars = int(doorwayde_config.get_value("CAVA_BARS", 16))
        range_val = int(doorwayde_config.get_value("CAVA_RANGE", 15))
        channels = doorwayde_config.get_value("CAVA_CHANNELS", "stereo")
        reverse = doorwayde_config.get_value("CAVA_REVERSE", 0)
        try:
            reverse = int(reverse)
        except Exception:
    ...
    def _create_cava_config(self, bars=16, range_val=15, channels="stereo", reverse=0, prefix=""):
        """Create cava configuration file with channels and reverse support, using DoorwaydeConfig with or without prefix as appropriate"""
        doorwayde_config = DoorwaydeConfig()

        if prefix:
            config_channels = doorwayde_config.get_value(f"CAVA_{prefix}_CHANNELS")
            config_reverse = doorwayde_config.get_value(f"CAVA_{prefix}_REVERSE")
        else:
            config_channels = doorwayde_config.get_value("CAVA_CHANNELS")
            config_reverse = doorwayde_config.get_value("CAVA_REVERSE")
...
class CavaClient:
    ...
    @staticmethod
    def parse_command_config(doorwayde_config, command, args):
        ...
        if hasattr(args, "bar_array") and args.bar_array:
            ...
            bar_array = doorwayde_config.get_value(f"{prefix}_BAR_ARRAY")
            if bar_array and isinstance(bar_array, list):
                bar_chars = bar_array
            else:
                bar_chars = args.bar or doorwayde_config.get_value(f"{prefix}_BAR", "▁▂▃▄▅▆▇█")
        ...
        width = (
            ...
            else int(doorwayde_config.get_value(f"{prefix}_WIDTH", "0") or 0)
        )
        if not width:
        ...
        if args.stb is not None:
            ...
            standby_mode = doorwayde_config.get_value(f"{prefix}_STANDBY", "0")
...
def main():
    ...
    if args.command == "manager":
        ...
        bar_chars, width, standby_mode = CavaClient.parse_command_config(
            doorwayde_config, args.command, args
        )
