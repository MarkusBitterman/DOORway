...
cache_dir="${DOORWAYDE_CACHE_HOME:-$HOME/.cache/doorwayde}"
...
check_content() {
    ...
    if [[ $line == *"[[ binary data"* ]]; then
        ...
        local temp_preview="$XDG_RUNTIME_DIR/doorwayde/pastebin-preview_$img_idx"
...
cliphist_cmd() {
    if [[ $CLIPHIST_IMAGE_HISTORY != true ]]; then
        ...
        DOORWAYDE_CLIPHIST_IMAGE_ONLY=true cliphist.image.py
    fi
}
show_history() {
    local selected_item
...
ocr_scan() {
    ...
    local runtime_dir="${XDG_RUNTIME_DIR:-/run/user/${EUID}}/doorwayde"
    local image_path="${runtime_dir}/cliphist_ocr.png"
    local index
    index="$(DOORWAYDE_CLIPHIST_IMAGE_ONLY=1 "${LIB_DIR}/doorwayde/cliphist.image.py" | head -n1)"
    [[ -n $index ]] || {
...
main() {
    ...
    argparse_init "$@"
    argparse_program "doorwayde-shell cliphist"
    argparse_header "DOORwayDE Clipboard Manager"
