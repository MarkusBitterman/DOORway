...
fn_wallbash() {
    ...
    [ -f "$DOORWAYDE_STATE_HOME/state" ] && source "$DOORWAYDE_STATE_HOME/state"
    ...
    export WALLBASH_SCRIPTS confDir doorwaydeConfDir cacheDir thmbDir dcolDir iconsDir themesDir fontsDir wallbashDirs enableWallDcol DOORWAYDE_THEME_DIR DOORWAYDE_THEME GTK_ICON GTK_THEME CURSOR_THEME
...
set -a
source "$dcol_file"
if [ -f "$DOORWAYDE_THEME_DIR/theme.dcol" ] && [ "$enableWallDcol" -eq 0 ]; then
    source "$DOORWAYDE_THEME_DIR/theme.dcol"
    print_log -sec "wallbash" -stat "override" "dominant colors from $DOORWAYDE_THEME theme"
    print_log -sec "wallbash" -stat " NOTE" "Remove \"$DOORWAYDE_THEME_DIR/theme.dcol\" to use wallpaper dominant colors"
fi
...
if [ "$enableWallDcol" -eq 0 ] && [[ $reload_flag -eq 1 ]]; then
    print_log -sec "wallbash" -stat "apply $dcol_mode colors" "$DOORWAYDE_THEME theme"
    mapfile -d '' -t deployList < <(find -H "$DOORWAYDE_THEME_DIR" -type f -name "*.theme" -print0)
    while read -r pKey; do
        fKey="$(find -H "$DOORWAYDE_THEME_DIR" -type f -name "$(basename "${pKey%.dcol}.theme")")"
