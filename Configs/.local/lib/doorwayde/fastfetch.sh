...
image_dirs=()
doorwayde_distro_logo=$iconDir/Wallbash-Icon/distro/$LOGO
case $1 in
    logo)
        random() {
            (   
                ...
                if [ -n "$DOORWAYDE_THEME" ] && [ -d "$confDir/doorwayde/themes/$DOORWAYDE_THEME/logo" ]; then
                    image_dirs+=("$confDir/doorwayde/themes/$DOORWAYDE_THEME/logo")
                fi
                [ -f "$doorwayde_distro_logo" ] && echo "$doorwayde_distro_logo"
...
options:
  ...
  --theme   Display a logo inside the doorwayde theme directory
            ...
            for arg in "$@"; do
                case $arg in
                    ...
                    --prof)
                        ...
                        ;;
                    --os)
                        [ -f "$doorwayde_distro_logo" ] && image_dirs+=("$doorwayde_distro_logo")
                        ;;
                    --local)
                        image_dirs+=("$confDir/fastfetch/logo")
                        ;;
                    --wall)
                        ...
                        ;;
                    --theme) if
                        [ -n "$DOORWAYDE_THEME" ] && [ -d "$confDir/doorwayde/themes/$DOORWAYDE_THEME/logo" ]
                    then
                        image_dirs+=("$confDir/doorwayde/themes/$DOORWAYDE_THEME/logo")
                    fi ;;
                esac
