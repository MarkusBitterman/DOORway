...
landing_dir="${XDG_CACHE_HOME:-$HOME/.cache}/doorwayde/landing/fonts"
    ...
    find "$temp_dir" -type f | while read -r file; do
        case "$file" in
            ...
            *.tar.xz)
                ...
                ;;
            *.ttf | *.otf)
                mkdir -p "$font_dir/doorwayde"
                mv "$file" "$font_dir/doorwayde/$name.ttf"
                echo "[font] $name installed successfully. Please restart hyprlock to apply changes."
                notify-send -i "preferences-desktop-font" "DOORwayDE font" "$name Installed successfully"
                return 0
                ;;
            *)
        ...
        if ! cp -rn "$temp_dir/$name" "$font_dir"; then
            ...
            notify-send -i "preferences-desktop-font" "DOORwayDE font" "Failed to extract $file"
            return 1
        fi
        notify-send -i "preferences-desktop-font" "DOORwayDE font" "$name Installed successfully"
    done
