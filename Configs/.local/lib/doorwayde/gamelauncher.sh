...
if [ "$#" -lt 2 ] && [[ $1 =~ ^[0-9]+$ ]]; then
    ...
    argparse_program "doorwayde-shell gamelauncher"
    argparse_header "DOORwayDE Game Launcher"
...
case ${style:-5} in
    ...
    "monitor_info=(\(.width) \(.height) \(.scale) \(.x) \(.y)) reserved_info=(\(.reserved | join(" ")))"')"
        ...
        BG=$HOME/.local/share/doorwayde/rofi/assets/steamdeck_holographic.png
        BGfx=$HOME/.cache/doorwayde/landing/steamdeck_holographic_${monitor_width}x$monitor_height.png
        if [ ! -e "$BGfx" ]; then
...
case "$backend" in
    steam)
        backend_command=("${XDG_STATE_HOME:-$HOME/.local/state}/doorwayde/python_env/bin/python" "$LIB_DIR/doorwayde/gamelauncher/steam.py" --rofi-string)
        ;;
    lutris)
        backend_command=("${XDG_STATE_HOME:-$HOME/.local/state}/doorwayde/python_env/bin/python" "$LIB_DIR/doorwayde/gamelauncher/lutris.py" --rofi-string)
        ;;
    *)
        backend_command=("${XDG_STATE_HOME:-$HOME/.local/state}/doorwayde/python_env/bin/python" "$LIB_DIR/doorwayde/gamelauncher/catalog.py" --rofi-string)
