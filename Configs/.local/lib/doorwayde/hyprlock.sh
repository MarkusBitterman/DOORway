...
if ! source "$(which doorwayde-shell)"; then
    echo "Error: doorwayde-shell not found."
    echo "Is DOORwayDE installed?"
    exit 1
fi
...
cacheDir="${DOORWAYDE_CACHE_HOME:-"$XDG_CACHE_HOME/doorwayde"}"
WALLPAPER="$cacheDir/wall.set"
HYPRLOCK_SCOPE_NAME="doorwayde-${XDG_SESSION_DESKTOP:-unknown}-lockscreen.scope"
...
fn_background() {
    ...
    if [ "$is_video" -eq 1 ]; then
        ...
        cached_thumb="$DOORWAYDE_CACHE_HOME/wallpapers/$(${hashMech:-sha1sum} "$WP" | cut -d' ' -f1).png"
...
fn_profile() {
    ...
    if [ -f "$HOME/.face.icon" ]; then
        ...
        cp "$XDG_DATA_HOME/icons/Wallbash-Icon/doorwayde.png" "$profilePath.png"
    fi
    return 0
}
fn_mpris() {
    ...
    if [[ $player_status == "Playing" ]]; then
        ...
        if [ -f "$HOME/.face.icon" ]; then
            ...
            if ! cmp -s "$XDG_DATA_HOME/icons/Wallbash-Icon/doorwayde.png" "$THUMB.png"; then
                cp "$XDG_DATA_HOME/icons/Wallbash-Icon/doorwayde.png" "$THUMB.png"
                reload_hyprlock
            fi
        fi
        exit 1
...
fn_cava() {
    ...
    config_file="$XDG_RUNTIME_DIR/doorwayde/cava.hyprlock"
    if [ "$(pgrep -c -f "cava -p $config_file")" -eq 0 ]; then
        trap 'rm -f ${tempFile}' EXIT
        "${LIB_DIR}/doorwayde/cava.py" hyprlock >$tempFile 2>&1
    fi
}
fn_art() {
...
find_filepath() {
    ...
    local search_dirs=(
        ...
        "${XDG_CONFIG_HOME:-$HOME/.config}/doorwayde/hyprlock"
        "$HYPRLOCK_CONF_DIR")
...
$layout_items"
    selected_layout=$(awk -F/ '{print $NF}' <<<"$layout_items" | rofi -dmenu -i -select "$HYPRLOCK_LAYOUT" \
        ...
        -on-selection-changed 'doorwayde-shell hyprlock.sh --test-preview  "{entry}"' \
    ...
    "$LIB_DIR/doorwayde/font.sh" resolve "$hyprlock_conf_path"
    fn_profile
...
layout_test() {
    ...
    sleep 2
    local temp_path="$XDG_RUNTIME_DIR/doorwayde/hyprlock-test.conf"
...
rofi_test_preview() {
    ...
    local unit_name="doorwayde-${XDG_SESSION_DESKTOP:-unknown}-lockscreen-preview.scope"
...
generate_conf() {
    ...
    local doorwayde_hyprlock_conf=${SHARE_DIR:-$XDG_DATA_HOME}/hypr/hyprlock.conf
...
#*│ \$XDG_CONFIG_HOME/hypr/doorwayde.conf,                                          │
...
#*┌──────────────────────────────────────────────────────────────────────────┐
#*│    All boilerplate configurations are handled by DOORwayDE                  │
#*└──────────────────────────────────────────────────────────────────────────┘

source = $doorwayde_hyprlock_conf
...
#┌────────────────────────────────────────────────────────────────────────────┐
#│ DOORwayDE also provides custom variables to extend hyprlock's functionality.  │
#│                                                                            │
#│   \$BACKGROUND_PATH                                                        │
...
#│   - if available, otherwise, it will show the DOORwayDE logo.                   │
#│                                                                            │
#│   \$PROFILE_IMAGE                                                          │
...
#│   - DOORwayDE will run 'font.sh resolve' to install the font for you.           │
...
fn_reload() {
    local layout="${HYPRLOCK_LAYOUT:-DOORwayDE}"
...
ensure_lockscreen_bg_exist() {
    if [ ! -f "$DOORWAYDE_CACHE_HOME/wallpapers/hyprlock.png" ]; then
        print_log -sec "hyprlock" -stat "setting" " $DOORWAYDE_CACHE_HOME/wallpapers/hyprlock.png"
        "${LIB_DIR}/doorwayde/wallpaper.sh" -s "$(readlink "$DOORWAYDE_THEME_DIR/wall.set")" --backend hyprlock
    fi
}

argparse_init "$@"
argparse_header "Hyprlock Manager"
argparse_program "doorwayde-shell hyprlock"
argparse_footer "Use 'doorwayde-shell hyprlock --help' for more information."
...
argparse "--reload,-r" "" "Regenerates hyprlock.conf using HYPRLOCK_LAYOUT (defaults to DOORwayDE)"
...
*)
    ensure_lockscreen_bg_exist
    check_and_sanitize_process
    "${LIB_DIR}/doorwayde/app2unit.sh" -u "$HYPRLOCK_SCOPE_NAME" -t scope -- hyprlock
    exit 0
    ;;
esac
