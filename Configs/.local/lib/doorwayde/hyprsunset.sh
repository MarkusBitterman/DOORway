...
sunsetConf="${XDG_STATE_HOME:-$HOME/.local/state}/doorwayde/hyprsunset"
default_temp=6000
default_gamma=100
temp_step=500
gamma_step=5
...
send_notification() {
    ...
    if [ -n "$message" ]; then
        notify-send -a "DOORwayDE Notify" -r 19 -t 800 -i redshift "$message" "$title"
    else
        notify-send -a "DOORwayDE Notify" -r 19 -t 800 -i redshift "$title"
    fi
}
