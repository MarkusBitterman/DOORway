#!/usr/bin/env python3
"""
DOORwayDE's LuaJIT environment manager
...
DOORWAYDE_STATE_DIR = os.path.join(XDG_STATE_HOME, "doorwayde")
LUA_ENV_DIR = os.path.join(XDG_STATE_HOME, "doorwayde", "lua_env")
...
ROCKS_SNAPSHOT = os.path.join(DOORWAYDE_STATE_DIR, "luarocks_env.json")
...
def ensure_state_dir():
    os.makedirs(DOORWAYDE_STATE_DIR, exist_ok=True)
...
def main():
    parser = argparse.ArgumentParser(
        prog="lua_env",
        usage="lua_env.py [command] <options>",
        description="DOORwayDE's LuaJIT environment manager",
