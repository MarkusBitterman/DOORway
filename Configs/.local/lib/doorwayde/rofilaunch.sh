...
if [[ -f "$DOORWAYDE_STATE_HOME/fullscreen_$r_mode" ]]; then
    hypr_width="0"
    wind_border="0"
fi
...
rofi -show "$r_mode" \
    ...
    -theme "$rofi_config" \
    -dump-theme | {
    grep -q "fullscreen.*true" && touch "$DOORWAYDE_STATE_HOME/fullscreen_$r_mode"
} || rm -f "$DOORWAYDE_STATE_HOME/fullscreen_$r_mode"
