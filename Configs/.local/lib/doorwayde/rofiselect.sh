...
rofiStyleDir="$SHARE_DIR/doorwayde/rofi/themes"
rofiAssetDir="$SHARE_DIR/doorwayde/rofi/assets"
...
if [ -n "$RofiSel" ]; then
    ...
    notify-send -a "DOORwayDE Alert" -r 2 -t 2200 -i "$rofiAssetDir/$RofiSel.png" " style $RofiSel applied..."
fi
if [ -n "$ROFI_LAUNCH_STYLE" ]; then
    notify-send -a "DOORwayDE Alert" -r 3 -u critical "Style: '$ROFI_LAUNCH_STYLE' is explicitly set, remove it in ~/.config/doorwayde/config.toml for changes to take effect."
fi
