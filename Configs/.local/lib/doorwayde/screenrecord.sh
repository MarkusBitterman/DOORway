...
if ! command -v "$RECORDER" &> /dev/null; then
    notify-send -a "DOORwayDE Alert" "No screen recorder found. Try installing wl-screenrec or wf-recorder."
...
Usage: 'doorwayde-shell screenrecord' [option]
...
Example:
        'doorwayde-shell screenrecord' --start -- --audio --codec libx264
...
handle_recording() {
    ...
    if [[ -z $GEOM ]]; then
        "$LIB_DIR/doorwayde/screenshot/grimblast" save active "$tmp_thumbnail"
    else
        grim -g "$GEOM" "$tmp_thumbnail"
    fi
    "$RECORDER" "${parameters[@]}" -f "$save_file_path"
    notify-send -a "DOORwayDE Alert" "$RECORDER: Recording saved at $save_file_path" -i "$tmp_thumbnail"
}
while [[ $# -gt 0 ]]; do
    case "$1" in
        --file)
        ...
        --quit)
            killall "$RECORDER"
            notify-send -a "DOORwayDE Alert" "Recording stopped"
            exit 0
            ;;
        --help)
            USAGE
