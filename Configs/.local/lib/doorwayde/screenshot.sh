...
[[ -f "${LIB_DIR}/doorwayde/shutils/l10n.sh" ]] && source "${LIB_DIR}/doorwayde/shutils/l10n.sh"
...
temp_screenshot=${XDG_RUNTIME_DIR:-/tmp}/doorwayde_screenshot.png
...
take_screenshot() {
    ...
    shift
    local extra_args=("$@")
    if "$LIB_DIR/doorwayde/screenshot/grimblast" "${extra_args[@]}" copysave "$mode" "$temp_screenshot"; then
        [[ ${SCREENSHOT_ANNOTATION_ENABLED} == false ]] && return 0
        if ! "$annotation_tool" "${annotation_args[@]}"; then
            send_notifs -r 9 -a "DOORwayDE Alert" "Screenshot Error" "Failed to open annotation tool"
            return 1
        fi
    else
        send_notifs -a "DOORwayDE Alert" "Screenshot Error" "Failed to take screenshot"
        return 1
    fi
}
ocr_screenshot() {
    ...
    local extra_args=("$@")
    if "$LIB_DIR/doorwayde/screenshot/grimblast" "${extra_args[@]}" copysave "$mode" "$temp_screenshot"; then
        source "${LIB_DIR}/doorwayde/shutils/ocr.sh"
        source ${XDG_STATE_HOME}/doorwayde/config
        ...
        if ! ocr_extract "$temp_screenshot"; then
            send_notifs -r 9 -a "DOORwayDE Alert" "OCR: extraction error" -e -i "dialog-error"
            return 1
        fi
    else
        send_notifs -a "DOORwayDE Alert" "OCR: screenshot error" -e -i "dialog-error"
        return 1
    fi
}
qr_screenshot() {
    ...
    local extra_args=("$@")
    if "$LIB_DIR/doorwayde/screenshot/grimblast" "${extra_args[@]}" copysave "$mode" "$temp_screenshot"; then
        source "${LIB_DIR}/doorwayde/shutils/qr.sh"
        ...
        if ! qr_extract "$temp_screenshot"; then
            send_notifs -r 9 -a "DOORwayDE Alert" "QR: extraction error" -e -i "dialog-error"
            return 1
        fi
    else
        send_notifs -a "DOORwayDE Alert" "QR: screenshot error" -e -i "dialog-error"
        return 1
    fi
}
...
if [ -f "$save_dir/$save_file" ]; then
    send_notifs -r 9 -a "DOORwayDE Alert" -i "$save_dir/$save_file" "saved in $save_dir"
fi
