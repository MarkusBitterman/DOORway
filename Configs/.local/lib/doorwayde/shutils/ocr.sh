...
ocr_extract() {
    ...
    for pkg in "${tesseract_packages[@]}"; do
        if ! pkg_installed "$pkg"; then
            notify-send -a "DOORwayDE Alert" "$(echo -e "OCR: required package is not installed\n $pkg")" -e -i "dialog-error"
            return 1
        fi
    done
    ...
    notify-send -a "DOORwayDE Alert" "$(echo -e "OCR: ${#tesseract_output} symbols recognized\n\nLanguages used ${tesseract_languages[@]/#/'\n '}")" -i "$image_path" -e -r 9
