...
kmenuDesk="$kmenuPath/doorwaydewallpaper.desktop"
tgtPath="$(dirname "$DOORWAYDE_THEME_DIR")"
get_themes
...
if [ ! -z "$setTheme" ] && [ ! -z "$setWall" ]; then
    ...
    if [[ ${wallHash[@]} == *"$inwallHash"* ]]; then
        notify-send -a "DOORwayDE Notify" -i "$thmbDir/$inwallHash.sqre" "Error" "Hash matched in $setTheme"
        exit 0
    fi
    ...
    notify-send -a "DOORwayDE Alert" -i "$thmbDir/$inwallHash.sqre" "Wallpaper set in $setTheme"
else
