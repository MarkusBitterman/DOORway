...
if [[ $DOORWAYDE_SHELL_INIT -ne 1 ]]; then
    eval "$(doorwayde-shell init)"
else
    export_doorwayde_config
fi

source "$LIB_DIR/doorwayde/wallpaper/help.sh"
source "$LIB_DIR/doorwayde/wallpaper/core.sh"
source "$LIB_DIR/doorwayde/wallpaper/select.sh"
...
run_cache_command() {
    ...
    case "$cache_mode" in
    current)
        "$LIB_DIR/doorwayde/wallpaper/cache.sh" commence
        ;;
    w | wall)
        ...
        "$LIB_DIR/doorwayde/wallpaper/cache.sh" commence -w "$cache_arg"
        ;;
    t | theme)
        if [ -z "$cache_arg" ]; then
        ...
        "$LIB_DIR/doorwayde/wallpaper/cache.sh" commence -t "$cache_arg"
        ;;
    f | full | all)
        "$LIB_DIR/doorwayde/wallpaper/cache.sh" commence -f
        ;;
    *)
...
setup_wallpaper_targets() {
    ...
    if [ "$set_as_global" == "true" ]; then
        wallSet="$DOORWAYDE_THEME_DIR/wall.set"
        wallCur="$DOORWAYDE_CACHE_HOME/wall.set"
        wallSqr="$DOORWAYDE_CACHE_HOME/wall.sqre"
        wallTmb="$DOORWAYDE_CACHE_HOME/wall.thmb"
        wallBlr="$DOORWAYDE_CACHE_HOME/wall.blur"
        wallQad="$DOORWAYDE_CACHE_HOME/wall.quad"
        wallDcl="$DOORWAYDE_CACHE_HOME/wall.dcol"
    elif [ -n "$wallpaper_backend" ]; then
        mkdir -p "$DOORWAYDE_CACHE_HOME/wallpapers"
        wallCur="$DOORWAYDE_CACHE_HOME/wallpapers/$wallpaper_backend.png"
        wallSet="$DOORWAYDE_THEME_DIR/wall.$wallpaper_backend.png"
    else
        wallSet="$DOORWAYDE_THEME_DIR/wall.set"
    fi
    if [ ! -e "$wallSet" ]; then
...
main() {
    ...
    if [ -n "$wallpaper_setter_flag" ]; then
        ...
        select)
            ...
            if [ "$output_flag" != "true" ]; then
                "$LIB_DIR/doorwayde/wallpaper/cache.sh" commence &>/dev/null &
            fi
            Wall_Select
    ...
    if [ -f "$LIB_DIR/doorwayde/wallpaper.$wallpaper_backend.sh" ] && [ -n "$wallpaper_backend" ]; then
        print_log -sec "wallpaper" "Using backend: $wallpaper_backend"
        "$LIB_DIR/doorwayde/wallpaper.$wallpaper_backend.sh" "$wallSet"
    else
        if command -v "wallpaper.$wallpaper_backend.sh" >/dev/null; then
            ...
            print_log -warn "wallpaper" "Created: $DOORWAYDE_CACHE_HOME/wallpapers/$wallpaper_backend.png instead"
        fi
    fi
    if [ "$wallpaper_setter_flag" == "select" ]; then
        if [ -e "$(readlink -f "$wallSet")" ]; then
            if [ "$set_as_global" == "true" ]; then
                notify-send -a "DOORwayDE Alert" -i "$selected_thumbnail" "$selected_wallpaper"
            else
                notify-send -a "DOORwayDE Alert" -i "$selected_thumbnail" "$selected_wallpaper set for $wallpaper_backend"
            fi
        else
            notify-send -a "DOORwayDE Alert" "Wallpaper not found"
        fi
    fi
}
if [ -z "$*" ]; then
