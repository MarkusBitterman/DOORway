...
notify-send "DEPRECATION NOTICE: swww backend is deprecated, please switch to awww or other supported backends. See 'doorwayde-shell wallpaper --help' for more info."

selected_wall="${1:-"$$DOORWAYDE_CACHE_HOME/wall.set"}"
lockFile="$XDG_RUNTIME_DIR/doorwayde/$(basename "$0").lock"
if [ -e "$lockFile" ]; then
    cat << EOF
