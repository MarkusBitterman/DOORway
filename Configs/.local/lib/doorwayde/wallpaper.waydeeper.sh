...
lockDir="${XDG_RUNTIME_DIR:-/run/user/$(id -u)}/doorwayde"
mkdir -p "$lockDir"
...
selected_wall="${1:-"$DOORWAYDE_CACHE_HOME/wall.set"}"
...
if [ "$is_video" -eq 1 ]; then
    ...
    if [ ! -f "$cached_thumb" ]; then
        ...
        notify-send -a "DOORwayDE Alert" "ERROR: failed to extract thumbnail from video"
        exit 1
    fi
...
if ! command -v waydeeper &>/dev/null; then
    ...
    notify-send -a "DOORwayDE Alert" "ERROR: waydeeper is not installed"
    exit 1
fi
...
if [ ! -d "$XDG_DATA_HOME/waydeeper/models/inpaint" ] && [ ! -d "$HOME/.local/share/waydeeper/models/inpaint" ]; then
    ...
    if ! waydeeper download-model inpaint; then
        ...
        notify-send -a "DOORwayDE Alert" "ERROR: failed to download waydeeper inpaint model"
        exit 1
    fi
fi
...
if [ -n "$WALLPAPER_WAYDEEPER_MODEL" ]; then
    ...
    if [ ! -f "$XDG_DATA_HOME/waydeeper/models/$WALLPAPER_WAYDEEPER_MODEL.onnx" ] && [ ! -f "$HOME/.local/share/waydeeper/models/$WALLPAPER_WAYDEEPER_MODEL.onnx" ]; then
        ...
        if ! waydeeper download-model $WALLPAPER_WAYDEEPER_MODEL; then
            ...
            notify-send -a "DOORwayDE Alert" "ERROR: failed to download waydeeper $WALLPAPER_WAYDEEPER_MODEL model"
            exit 1
        fi
    fi
fi
