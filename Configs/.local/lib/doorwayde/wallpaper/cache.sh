...
wallpaper_cache_bootstrap() {
	if [[ ${DOORWAYDE_SHELL_INIT:-0} -ne 1 ]]; then
...
wallpaper_cache_init() {
	[ -d "$DOORWAYDE_THEME_DIR" ] && cacheIn="$DOORWAYDE_THEME_DIR" || {
		echo "Error: DOORWAYDE_THEME_DIR not found!"
		return 1
	}
...
fn_wallcache() {
	...
	if [ "$is_video" -eq 1 ]; then
		if [ ! -e "$thmbDir/$x_hash.thmb" ] || [ ! -e "$thmbDir/$x_hash.sqre" ] || [ ! -e "$thmbDir/$x_hash.blur" ] || [ ! -e "$thmbDir/$x_hash.quad" ] || [ ! -e "$dcolDir/$x_hash.dcol" ]; then
			...
			notify-send -a "DOORwayDE wallpaper" "Extracting thumbnail from video wallpaper..."
	...
	while getopts "w:t:f" option; do
		case $option in
			w)
				...
				;;
			t)
				cacheIn="$(dirname "$DOORWAYDE_THEME_DIR")/$OPTARG"
				if [ ! -d "$cacheIn" ]; then
				...
				;;
			f)
				cacheIn="$(dirname "$DOORWAYDE_THEME_DIR")"
				mode="_force"
				;;
			*)
				echo "... invalid option ..."
