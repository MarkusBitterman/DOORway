...
Wall_Json() {
    setIndex=0
    [ ! -d "$DOORWAYDE_THEME_DIR" ] && echo "ERROR: \"$DOORWAYDE_THEME_DIR\" does not exist" && exit 0
    if [ -d "$DOORWAYDE_THEME_DIR/wallpapers" ]; then
        wallPathArray=("$DOORWAYDE_THEME_DIR/wallpapers")
    else
        wallPathArray=("$DOORWAYDE_THEME_DIR")
    fi
    wallPathArray+=("${WALLPAPER_CUSTOM_PATHS[@]}")
    [ -d "${XDG_PICTURES_DIR%/}/Wallpapers/${DOORWAYDE_THEME}" ] && wallPathArray+=("${XDG_PICTURES_DIR%/}/Wallpapers/${DOORWAYDE_THEME}")
    get_hashmap "${wallPathArray[@]}"
    wallListJson=$(printf '%s\n' "${wallList[@]}" | jq -R . | jq -s .)
    wallHashJson=$(printf '%s\n' "${wallHash[@]}" | jq -R . | jq -s .)
    jq -n --argjson wallList "$wallListJson" --argjson wallHash "$wallHashJson" --arg cacheHome "${DOORWAYDE_CACHE_HOME:-$HOME/.cache/doorwayde}" '
...
Wall_Cache() {
    ...
    if [ "$set_as_global" == "true" ]; then
        ...
        "$LIB_DIR/doorwayde/wallpaper/cache.sh" commence -w "${wallList[setIndex]}" &> /dev/null
        "$LIB_DIR/doorwayde/color.set.sh" "${wallList[setIndex]}" &
