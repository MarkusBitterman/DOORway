...
- [TODO] **Architecture-Overview.md** — the orchestrator chain (`~/.config/hypr/hyprland.lua` → `~/.local/share/doorway/hyprland.lua` → `env`, `variables`, `defaults`, `windowrules`, `dynamic`, `startup`, `workflows`, `finale`), the QuickShell surface map, the declarative systemd unit fleet, how `doorway-shell` and `doorwayctl` fit into the runtime, the IPC topology.
- [TODO] **Theming.md** — the two-system split: the committed `DoorwayPalette.qml` cartridge modes (shell) and the matugen wallpaper → Hyprland-border pipeline; how `Appearance.qml` maps modes to Material schemes.
