          ...
          options.doorway = {
            ...
            lock = {
              backend = lib.mkOption {
                ...
                default = "hyprlock";
                example = "doorway-lock";
                description = ''
                  Which locker lockscreen.sh launches. "doorway-lock" is the
                  QuickShell shader-screensaver lock (DOORway Lock); its wrapper
                  falls back to hyprlock automatically whenever the shell isn't
                  running, so it never fails open. "hyprlock" keeps the classic
                  locker and its layout presets.
                '';
              };
              layout = lib.mkOption {
                type = lib.types.enum [
          ...
          config = lib.mkIf cfg.enable {
            ...
            xdg.configFile =
              ...
              "hypr/hypridle.conf".text = ''
                ...
                ${
                  # doorway-lock lives inside the shell process — unlocking is an
                  # IPC call; the hyprlock-era pkill would kill the whole shell.
                  if cfg.lock.backend == "doorway-lock" then
                    "$UNLOCK_CMD = ${pkgs.quickshell}/bin/qs -c doorway ipc --any-display call lock unlock"
                  else
              ...
              "hypr/hyprsunset.conf".text = ''
                ...
                #
                # Intentionally SCHEDULELESS. The QuickShell Hyprsunset service is the sole
                # driver of temperature/gamma (seeded from doorway.blueLight into config.json,
                # and following weather-based sunset/sunrise when enabled). Defining profiles
                # here makes the hyprsunset daemon run its OWN fixed schedule and periodically
                # re-assert it, reverting QuickShell's setting — e.g. the night light flicking
                # on at weather-sunset then off ~a minute later, because the daemon's day
              ...
              "hypr/userprefs.lua".text = ''
                ...
                -- Locker for Hyprland-spawned lockscreen.sh (Super+L); the idle
                -- path gets the same value via the doorway-idle unit env.
                hl.env("DOORWAY_LOCKSCREEN", "${cfg.lock.backend}")
                hl.config({
                    input = {
            ...
            systemd.user.services = {
              ...
              doorway-idle = lib.mkIf cfg.idle.enable (mkDoorwayService {
                ...
                # hypridle's lock_cmd runs in THIS unit's env, not Hyprland's —
                # the locker choice must be injected here for the idle path
                environment = [ "DOORWAY_LOCKSCREEN=${cfg.lock.backend}" ];
              });
            ...
            home.activation.doorwayJsonConfig = lib.mkIf cfg.enable (
              lib.hm.dag.entryAfter [ "writeBoundary" "linkGeneration" ] (
                let
                  ...
                  temperature = cfg.blueLight.temperature;
                ...
                ''
                  ...
                  ${pkgs.jq}/bin/jq \
                    ...
                    --argjson colorTemperature ${toString temperature} \
                    --arg topLeftIcon "${topLeftIcon}" \
                    '.light.night.from = $from | .light.night.to = $to | .light.night.useWeatherTimes = $useWeather | .light.night.colorTemperature = $colorTemperature | .bar.topLeftIcon = $topLeftIcon' \
